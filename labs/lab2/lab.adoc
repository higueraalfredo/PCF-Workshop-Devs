...
As an instance reaches the `running` state, its https://docs.cloudfoundry.org/concepts/diego/diego-architecture.html[Diego] registers that instance in the routing table assigned to its route by sending a message to Cloud Foundry's message bus.
